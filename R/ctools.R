...
#
# cgen is free software: you can redistribute it and/or modify it
# under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 2 of the License, or
# (at your option) any later version.
#
# cgen is distributed in the hope that it will be useful, but
...
if(.Call("check_openmp",PACKAGE="cgen")) 
...
get_max_threads <- function() .Call("get_max_threads",PACKAGE="cgen")

# get_num_threads

get_num_threads <- function() options()$cgen.threads

# check_limit_threads

#get_limit_threads <- function() .Call("get_limit_threads",PACKAGE="cgen")

# set.threads

set_num_threads <- function(x) {

if(!.Call("check_openmp",PACKAGE="cgen")) { cat("OpenMP is not available, threads set to 1 \n") } else {
  if(length(x)!=1) stop("must be a scalar")
  t = as.integer(round(x,digits=0))
  if(t<=0) t=1

  options(cgen.threads=t)
  cat("Number of threads set to ",t,"\n")
  }
...
ccov <- function(X,lambda=0, w=NULL, cor=FALSE){
  ...
  .Call( "ccov", X,lambda, as.numeric(w), as.integer(cor), options()$cgen.threads, PACKAGE = "cgen" )
...
cgrm.A <- function(X, lambda=0, yang=FALSE){
         ...
         if(lambda>1) lambda=1
         if(lambda<0) lambda=0        
	 .Call( "camat", X, lambda, yang, options()$cgen.threads ,PACKAGE = "cgen" )
}
...
cgrm.D <- function(X, lambda=0){
         ...
         if(lambda>1) lambda=1
         if(lambda<0) lambda=0
	.Call( "cdmat", X, lambda, options()$cgen.threads ,PACKAGE = "cgen" )
}
...
cgrm <- function(X, w = NULL, lambda=0){
	 ...
	 .Call( "cgrm", X, w,isw, lambda, options()$cgen.threads ,PACKAGE = "cgen" )
}
...
# parallel crossproduct operator
 ...
 if(a == "matrix"){
   if(b == "matrix") {.Call( "ccp_dense_dense", x,y,options()$cgen.threads ,PACKAGE = "cgen" ) } 
     else { .Call( "ccp_dense_sparse", x,y,PACKAGE = "cgen" ) } 
 } else {
   if(b == "matrix") { .Call( "ccp_sparse_dense", x,y,PACKAGE = "cgen" ) }
     else { .Call( "ccp_sparse_sparse", x,y,PACKAGE = "cgen" ) } 
 } 
...
n_vec[1:(folds-1)] <- round(length(isy)/folds,dig=0)
...
cmaf <- function(X){
   maf <- .Call( "cmaf", X ,PACKAGE = "cgen" )[1,]
...
ccross <- function(X,D=NULL){
	 ...
	 .Call( "ccross", X, D, options()$cgen.threads, PACKAGE = "cgen" )
}
...
`%**%` <- function(S, power) with(eigen(S), ccross(vectors,values^power)) 
...
ccolmv <- function(X,var=FALSE){
     ...
     .Call( "ccolmv", X,var ,PACKAGE = "cgen" )[1,]
...
randm <- function(n,p) array(rnorm(n*p),dim=c(n,p))
